...
```
┌──────────────┐         ┌──────────────────┐
│  Endpoint    │  TCP    │  Splunk Indexer   │
│  (Forwarder) │ ──9997──▶  (Search Head)   │
└──────────────┘         └──────────────────┘
```
...
Take these screenshots to make your GitHub portfolio visually compelling and demonstrate working configurations:

| Screenshot | What to Capture | Filename |
|:---|:---|:---|
| Splunk Login | Splunk Web login page | `screenshots/splunk-login.png` |
| Receiving Port | Settings → Receiving showing port 9997 | `screenshots/splunk-receiving-port.png` |
| Data Ingestion | Search results showing ingested Windows events | `screenshots/splunk-data-ingestion.png` |
| SPL Search | A query with results (failed logins is most impactful) | `screenshots/splunk-spl-search.png` |
| Login Timechart | Time-series visualization of success vs. failure | `screenshots/splunk-login-timechart.png` |
| Security Dashboard | Full dashboard with all panels populated | `screenshots/splunk-security-dashboard.png` |
| Alert Configuration | Alert settings page with all fields configured | `screenshots/splunk-brute-force-alert.png` |
| Alert Triggered | Triggered Alerts page showing the alert fired | `screenshots/splunk-alert-triggered.png` |
...
├── README.md                          # This file
├── diagrams/
│   ├── splunk-data-flow.svg           # Data flow architecture
│   └── splunk-architecture.svg        # Component architecture
├── configs/
│   └── inputs.conf                    # Forwarder configuration
├── queries/
│   └── spl-queries.md                 # All SPL queries with explanations
├── screenshots/
│   ├── splunk-login.png
│   ├── splunk-receiving-port.png
│   ├── splunk-data-ingestion.png
│   ├── splunk-spl-search.png
│   ├── splunk-login-timechart.png
│   ├── splunk-security-dashboard.png
│   ├── splunk-brute-force-alert.png
│   └── splunk-alert-triggered.png
└── docs/
    └── troubleshooting.md             # Common issues and fixes
```

### How to Present This Lab in Interviews

1. **Start with the business problem** — "Organizations need centralized visibility into security events to detect threats in real time."
2. **Explain your architecture** — "I deployed Splunk Free on an Azure VM with a Universal Forwarder collecting Windows Security, System, and Application logs over TCP 9997."
3. **Walk through a detection scenario** — "I wrote an SPL query that detects brute force attempts by counting failed logins per source IP, and created an automated alert that fires when any IP exceeds 10 failures in 15 minutes."
4. **Show the dashboard** — "This is my SOC dashboard with six panels providing real-time visibility into authentication patterns, privilege escalation, and persistence mechanisms."
5. **Connect to frameworks** — "These detections map to MITRE ATT&CK techniques: T1110 (Brute Force), T1078 (Valid Accounts), T1543 (Create or Modify System Process)."

---

## 📝 Resume Bullet Points

Use these accomplishment-driven bullet points on your resume. Each one is tailored to pass ATS (Applicant Tracking System) filters and highlight measurable outcomes:

> - Deployed and configured **Splunk SIEM** on Azure infrastructure, ingesting **Windows Security, System, and Application logs** via Universal Forwarder over TCP 9997
>
> - Authored **SPL detection queries** targeting brute force attacks (Event ID 4625), privilege escalation (Event ID 4672), unauthorized account creation (Event ID 4720), and persistence mechanisms (Event ID 7045)
>
> - Built a **multi-panel Security Operations Dashboard** providing real-time visibility into authentication trends, failed login sources, privilege changes, and service installations
>
> - Created **automated alerting rules** with threshold-based triggers for brute force detection, reducing mean time to detect (MTTD) for credential-based attacks
>
> - Configured **Splunk Universal Forwarder** with custom `inputs.conf` for targeted log collection across three Windows Event Log channels
>
> - Mapped detection queries to **MITRE ATT&CK** techniques including T1110 (Brute Force), T1078 (Valid Accounts), and T1543 (Create or Modify System Process)
